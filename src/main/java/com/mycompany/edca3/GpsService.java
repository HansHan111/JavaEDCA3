...
public class GpsService {
    ...
    /**
     * Reads binary data from the gps.dat file and plays that data back at the
     * intervals given in the data set.
     */
    private void start() {
    ...
    /**
     * Asynchronous runnable class for playing back events at the corresponding
     * time.
     */
    private class GpsInput extends TimerTask {
        ...
        public void run() {
            ...
            stream.send(new GpsEvent("Tracker" + id, event[0].doubleValue(), event[1].doubleValue(),
                    event[2].doubleValue()));
